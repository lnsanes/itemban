...
public final class AdminNetwork {
    ...
    public static void requestOpen(ServerPlayer player) {
        if (!BanPermission.hasBan(player)) {
            player.sendMessage(new net.minecraft.network.chat.TextComponent("§c没有 itemban.ban 权限"), net.minecraft.Util.NIL_UUID);
            return;
        }
        ...
        static void handle(ActionC2S msg, Supplier<NetworkEvent.Context> ctx) {
            ctx.get().enqueueWork(() -> {
                ...
                if (!AdminSessions.isAuthorized(player.getUUID())) {
                    player.sendMessage(new net.minecraft.network.chat.TextComponent("§c管理会话已过期，请重新输入 /itemban gui"), net.minecraft.Util.NIL_UUID);
                    return;
                }
