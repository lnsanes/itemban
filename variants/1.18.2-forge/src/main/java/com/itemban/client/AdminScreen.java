...
public class AdminScreen extends Screen {
    ...
    public AdminScreen(String json) {
        super(new TextComponent("ItemBan 管理"));
    ...
    protected void init() {
        ...
        this.idBox = new EditBox(this.font, left, 22, fieldW, 18, new TextComponent("ID"));
        this.idBox.setMaxLength(256);
        this.idBox.setResponder(v -> refreshHint());
        this.addRenderableWidget(this.idBox);

        this.nbtBox = new EditBox(this.font, left, 44, fieldW, 18, new TextComponent("NBT"));
        ...
        int y = 20;
        int bw = 88;
        this.addRenderableWidget(new Button(x, y, bw, 20, new TextComponent("添加物品"), b -> add("add_item", false)));
        this.addRenderableWidget(new Button(x + 90, y, bw, 20, new TextComponent("添加方块"), b -> add("add_block", true)));
        this.addRenderableWidget(new Button(x, y + 22, bw, 20, new TextComponent("重载配置"), b -> AdminNetwork.sendAction("reload", "")));
        this.addRenderableWidget(new Button(x + 90, y + 22, bw, 20, new TextComponent("关闭"), b -> onClose()));
    ...
    private Button toggle(String label, String op, String key, int x, int y) {
        boolean on = bool(key);
        return new Button(x, y, 88, 20, new TextComponent(label + (on ? " §a开" : " §c关")), b -> {
    ...
    private void error(String msg) {
        if (this.minecraft != null && this.minecraft.player != null) {
            this.minecraft.player.displayClientMessage(new TextComponent("§c" + msg), false);
        }
    }
    ...
    public void render(PoseStack pose, int mouseX, int mouseY, float partialTick) {
        ...
        this.font.draw(pose, "ItemBan 游戏内管理  keyId=" + str("keyId"), 12, 8, 0xFFE082);

        int left = listLeft();
        int top = listTop();
        int right = listRight();
        int bottom = listBottom();
        fill(pose, left - 2, top - 2, right + 2, bottom, 0xC0000000);
        ...
        for (int i = 0; i < this.rows.size(); i++) {
            ...
            this.font.draw(pose, shown, left, y + 4, row.color);
            if (row.removable()) {
                int by = y + 2;
                boolean hover = mouseX >= btnX && mouseX < btnX + BTN_W && mouseY >= by && mouseY < by + 14
                        && mouseY >= top && mouseY < bottom;
                fill(pose, btnX, by, btnX + BTN_W, by + 14, hover ? 0xFFC0392B : 0xFF8B1E1E);
                this.font.draw(pose, "移除", btnX + 4, by + 3, 0xFFFFFF);
            }
        }

        if (bar) {
            int barX = right - BAR_W;
            fill(pose, barX, top, right, bottom, 0x33000000);
            ...
            fill(pose, barX, thumbY, right, thumbY + thumbH, 0xFFBBBBBB);
        }
    }
