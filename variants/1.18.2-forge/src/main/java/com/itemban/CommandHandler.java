                ...
                .then(Commands.argument("item", StringArgumentType.greedyString())
                    .executes(ctx -> {
                        ...
                        String msg = nbtString != null 
                            ? "已添加黑名单物品: " + itemId + " (带 NBT)，已更新配方表（移除 " + stripped + " 条）"
                            : "已添加黑名单物品: " + itemId + "，已更新配方表（移除 " + stripped + " 条）";
            ...
            .then(Commands.literal("remove")
                .then(Commands.argument("item", StringArgumentType.greedyString())
                    .executes(ctx -> {
                        ...
                        int stripped = RecipeStripper.applyFromSnapshot(ctx.getSource().getServer());
                        String msg = nbtString != null 
                            ? "已移除黑名单物品: " + itemId + " (指定 NBT)，已恢复对应配方（当前仍过滤 " + stripped + " 条）"
                            : "已移除黑名单物品: " + itemId + " (所有规则)，已恢复对应配方（当前仍过滤 " + stripped + " 条）";
                        ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent(msg), true);
                        return 1;
                    })))
                ...
                .then(Commands.literal("add")
                    .then(Commands.argument("block", StringArgumentType.greedyString())
                        .executes(ctx -> {
                            ...
                            ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§a已将方块 " + input + " 加入方块黑名单"), true);
                            return 1;
                        })))
                .then(Commands.literal("remove")
                    .then(Commands.argument("block", StringArgumentType.greedyString())
                        .executes(ctx -> {
                            ...
                            ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§c已将方块 " + input + " 从方块黑名单移除"), true);
                            return 1;
                        })))
                .then(Commands.literal("list")
                    .executes(ctx -> {
