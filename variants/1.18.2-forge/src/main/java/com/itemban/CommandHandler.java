        ...
        dispatcher.register(Commands.literal("itemban")
            ...
            .then(Commands.literal("add")
                .then(Commands.argument("item", StringArgumentType.greedyString())
                    .executes(ctx -> {
                        ...
                        ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent(msg), true);
                        return 1;
                    })))
            ...
            .then(Commands.literal("list")
                .executes(ctx -> {
                    ...
                    ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent(sb.toString()), true);
                    return 1;
                }))
            .then(Commands.literal("reload")
                .executes(ctx -> {
                    ...
                    int stripped = RecipeStripper.applyFromSnapshot(ctx.getSource().getServer());
                    ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("黑名单与管理密钥已热重载，已更新配方表（移除 " + stripped + " 条）"), true);
                    return 1;
                }))
            .then(Commands.literal("gui")
                .executes(ctx -> {
                    ...
                    if (player == null) {
                        ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§c请在游戏内使用 /itemban gui"), false);
                        return 0;
                    }
                    AdminNetwork.requestOpen(player);
                    ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§7正在核对管理模组密钥与哈希…"), false);
                    return 1;
                }))
            ...
            .then(Commands.literal("announce")
                .then(Commands.literal("on")
                    .executes(ctx -> {
                        ...
                        ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§a已开启聊天栏公示功能"), true);
                        return 1;
                    }))
                .then(Commands.literal("off")
                    .executes(ctx -> {
                        ConfigHandler.setPublicAnnounce(false);
                        ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§c已关闭聊天栏公示功能"), true);
                        return 1;
                    })))
            .then(Commands.literal("autoban")
                .then(Commands.literal("on")
                    .executes(ctx -> {
                        ...
                        ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§a已开启自动踢出功能"), true);
                        return 1;
                    }))
                .then(Commands.literal("off")
                    .executes(ctx -> {
                        ConfigHandler.setAutoBanOnViolation(false);
                        ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§c已关闭自动踢出功能"), true);
                        return 1;
                    })))
            .then(Commands.literal("dropdetect")
                .then(Commands.literal("on")
                    .executes(ctx -> {
                        ...
                        ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§a已开启掉落物检测"), true);
                        return 1;
                    }))
                .then(Commands.literal("off")
                    .executes(ctx -> {
                        ConfigHandler.setDetectDroppedItems(false);
                        ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§c已关闭掉落物检测"), true);
                        return 1;
                    })))
            .then(Commands.literal("blockscan")
                .then(Commands.literal("on")
                    .executes(ctx -> {
                        ...
                        ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§a已开启世界方块检测"), true);
                        return 1;
                    }))
                .then(Commands.literal("off")
                    .executes(ctx -> {
                        ConfigHandler.setDetectWorldBlocks(false);
                        ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§c已关闭世界方块检测"), true);
                        return 1;
                    })))
                ...
                .then(Commands.literal("add")
                    .then(Commands.argument("block", StringArgumentType.greedyString())
                        .executes(ctx -> {
                            ...
                            ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§a已将方块 " + parsed.id + (parsed.nbt == null ? "" : parsed.nbt) + " 加入方块黑名单"), true);
                            return 1;
                        })))
                .then(Commands.literal("remove")
                    .then(Commands.argument("block", StringArgumentType.greedyString())
                        .executes(ctx -> {
                            ...
                            ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§c已将方块 " + parsed.id + (parsed.nbt == null ? "" : parsed.nbt) + " 从方块黑名单移除"), true);
                            return 1;
                        })))
                .then(Commands.literal("list")
                    .executes(ctx -> {
                ...
                .then(Commands.literal("add")
                    .then(Commands.argument("item", StringArgumentType.greedyString())
                        .executes(ctx -> {
                            ...
                            ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§a已将 " + itemId + " 加入审计排除列表"), true);
                            return 1;
                        })))
                .then(Commands.literal("remove")
                    .then(Commands.argument("item", StringArgumentType.greedyString())
                        .executes(ctx -> {
                            ...
                            ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§c已将 " + itemId + " 从审计排除列表移除"), true);
                            return 1;
                        })))
                .then(Commands.literal("list")
                    .executes(ctx -> {
            ...
            .then(Commands.literal("web")
                .executes(ctx -> {
                    if (!ConfigHandler.webEnabled) {
                        ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§c网页管理已关闭，请在 config/ItemBan/config.json 将 webEnabled 设为 true 后重启"), false);
                        return 0;
                    }
                    ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent(ConfigHandler.webInfoMessage()), false);
                    return 1;
                })
                .then(Commands.literal("password")
                    .then(Commands.argument("password", StringArgumentType.greedyString())
                        .executes(ctx -> {
                            ...
                            if (error != null) {
                                ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§c" + error), false);
                                return 0;
                            }
                            ctx.getSource().sendSuccess(new net.minecraft.network.chat.TextComponent("§a已设置 admin 正式密码（已哈希存储）"), true);
                            return 1;
                        }))))
        );
    }
