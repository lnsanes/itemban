            ...
            .then(Commands.literal("add")
                .then(Commands.argument("item", StringArgumentType.greedyString())
                    .executes(ctx -> {
                        ...
                        ctx.getSource().sendSuccess(new StringTextComponent(msg), true);
                        return 1;
                    })))
            ...
            .then(Commands.literal("announce")
                .then(Commands.literal("on")
                    .executes(ctx -> {
                        ...
                        ctx.getSource().sendSuccess(new StringTextComponent("§a已开启聊天栏公示功能"), true);
                        return 1;
                    }))
                .then(Commands.literal("off")
                    .executes(ctx -> {
                        ConfigHandler.setPublicAnnounce(false);
                        ctx.getSource().sendSuccess(new StringTextComponent("§c已关闭聊天栏公示功能"), true);
                        return 1;
                    })))
            .then(Commands.literal("autoban")
                .then(Commands.literal("on")
                    .executes(ctx -> {
                        ...
                        ctx.getSource().sendSuccess(new StringTextComponent("§a已开启自动踢出功能"), true);
                        return 1;
                    }))
                .then(Commands.literal("off")
                    .executes(ctx -> {
                        ConfigHandler.setAutoBanOnViolation(false);
                        ctx.getSource().sendSuccess(new StringTextComponent("§c已关闭自动踢出功能"), true);
                        return 1;
                    })))
            .then(Commands.literal("dropdetect")
                .then(Commands.literal("on")
                    .executes(ctx -> {
                        ...
                        ctx.getSource().sendSuccess(new StringTextComponent("§a已开启掉落物检测"), true);
                        return 1;
                    }))
                .then(Commands.literal("off")
                    .executes(ctx -> {
                        ConfigHandler.setDetectDroppedItems(false);
                        ctx.getSource().sendSuccess(new StringTextComponent("§c已关闭掉落物检测"), true);
                        return 1;
                    })))
            .then(Commands.literal("blockscan")
                .then(Commands.literal("on")
                    .executes(ctx -> {
                        ...
                        ctx.getSource().sendSuccess(new StringTextComponent("§a已开启世界方块检测"), true);
                        return 1;
                    }))
                .then(Commands.literal("off")
                    .executes(ctx -> {
                        ConfigHandler.setDetectWorldBlocks(false);
                        ctx.getSource().sendSuccess(new StringTextComponent("§c已关闭世界方块检测"), true);
                        return 1;
                    })))
                ...
                .then(Commands.literal("add")
                    .then(Commands.argument("block", StringArgumentType.greedyString())
                        .executes(ctx -> {
                            ...
                            ctx.getSource().sendSuccess(new StringTextComponent("§a已将方块 " + parsed.id + (parsed.nbt == null ? "" : parsed.nbt) + " 加入方块黑名单"), true);
                            return 1;
                        })))
                .then(Commands.literal("remove")
                    .then(Commands.argument("block", StringArgumentType.greedyString())
                        .executes(ctx -> {
                            ...
                            ctx.getSource().sendSuccess(new StringTextComponent("§c已将方块 " + parsed.id + (parsed.nbt == null ? "" : parsed.nbt) + " 从方块黑名单移除"), true);
                            return 1;
                        })))
                .then(Commands.literal("list")
                    .executes(ctx -> {
                        StringBuilder sb = new StringBuilder("§6方块黑名单:\n");
                        for (ConfigHandler.BlacklistRule rule : ConfigHandler.getBlockBlacklistRules()) {
                ...
                .then(Commands.literal("add")
                    .then(Commands.argument("item", StringArgumentType.greedyString())
                        .executes(ctx -> {
                            ...
                            ctx.getSource().sendSuccess(new StringTextComponent("§a已将 " + itemId + " 加入审计排除列表"), true);
                            return 1;
                        })))
                .then(Commands.literal("remove")
                    .then(Commands.argument("item", StringArgumentType.greedyString())
                        .executes(ctx -> {
                            ...
                            ctx.getSource().sendSuccess(new StringTextComponent("§c已将 " + itemId + " 从审计排除列表移除"), true);
                            return 1;
                        })))
                .then(Commands.literal("list")
                    .executes(ctx -> {
            ...
            .then(Commands.literal("web")
                .executes(ctx -> {
                    if (!ConfigHandler.webEnabled) {
                        ctx.getSource().sendSuccess(new StringTextComponent("§c网页管理已关闭，请在 config/ItemBan/config.json 将 webEnabled 设为 true 后重启"), false);
                        return 0;
                    }
                    ctx.getSource().sendSuccess(new StringTextComponent(ConfigHandler.webInfoMessage()), false);
                    return 1;
                })
                .then(Commands.literal("password")
                    .then(Commands.argument("password", StringArgumentType.greedyString())
                        .executes(ctx -> {
                            ...
                            if (error != null) {
                                ctx.getSource().sendSuccess(new StringTextComponent("§c" + error), false);
                                return 0;
                            }
                            ctx.getSource().sendSuccess(new StringTextComponent("§a已设置 admin 正式密码（已哈希存储）"), true);
                            return 1;
                        }))))
        );
    }
