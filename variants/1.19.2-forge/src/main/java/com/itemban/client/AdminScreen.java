...
public class AdminScreen extends Screen {
    ...
    protected void init() {
        ...
        this.idBox = new EditBox(this.font, left, 22, fieldW, 18, Component.literal("ID"));
        this.idBox.setMaxLength(256);
        this.idBox.setResponder(v -> refreshHint());
        this.addRenderableWidget(this.idBox);

        this.nbtBox = new EditBox(this.font, left, 44, fieldW, 18, Component.literal("NBT"));
        ...
        int y = 20;
        int bw = 88;
        this.addRenderableWidget(new Button(x, y, bw, 20, Component.literal("添加物品"), b -> add("add_item", false)));
        this.addRenderableWidget(new Button(x + 90, y, bw, 20, Component.literal("添加方块"), b -> add("add_block", true)));
        this.addRenderableWidget(new Button(x, y + 22, bw, 20, Component.literal("重载配置"), b -> AdminNetwork.sendAction("reload", "")));
        this.addRenderableWidget(new Button(x + 90, y + 22, bw, 20, Component.literal("关闭"), b -> onClose()));
    ...
    private Button toggle(String label, String op, String key, int x, int y) {
        boolean on = bool(key);
        return new Button(x, y, 88, 20, Component.literal(label + (on ? " §a开" : " §c关")), b -> {
    ...
    private void error(String msg) {
        if (this.minecraft != null && this.minecraft.player != null) {
            this.minecraft.player.displayClientMessage(Component.literal("§c" + msg), false);
        }
    }
